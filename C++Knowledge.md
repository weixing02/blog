...
#### 函数模板template和函数重载的区别：

- 首先函数重载是为了在不修改函数名的情况下，实现对参数类型不同、参数个数不同、参数顺序不同的同名函数调用提供不同实现；但是仍然需要针对不同的情况实现多个函数；

- 利用函数模板template的可以实现只用一个函数就能针对不同类型函数不同实现，但是限制在于对于参数类型不同和参数顺序不同的情况就只能写多个函数实现了；
